...
public class PlagiarismTool {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (files.size() < 2) {
            System.out.println("Not enough files in the folder to compare. Need at least 2 files.");
            return;
        }

        Path baseFile = files.get(0);
        String baseContent = Files.readString(baseFile);
        String normBaseContent = normalize(baseContent);
        List<KGram> baseKGrams = generateKGrams(normBaseContent);
        List<Integer> baseHashes = getHashes(baseKGrams);
        List<Integer> baseFingerprints = generateFingerprints(baseHashes);
        ...
        for (int i = 1; i < files.size(); i++) {
            ...
            String normOtherContent = normalize(otherContent);

            List<KGram> otherKGrams = generateKGrams(normOtherContent);
            List<Integer> otherHashes = getHashes(otherKGrams);
            List<Integer> otherFingerprints = generateFingerprints(otherHashes);
